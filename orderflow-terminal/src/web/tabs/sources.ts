  ...
  async function renderDiag(): Promise<void> {
    ...
    try {
      ...
      diagBox.innerHTML = `
        ...
        Закрытые 1m-свечи из потока vs REST: ${v.candles.compared ? `OHLC ${v.candles.ohlcMatches}/${v.candles.compared}, объём ${v.candles.volumeWithin}/${v.candles.compared}${v.candles.lastDiff ? ' · ' + esc(String(v.candles.lastDiff)) : ''}` : 'сверка ещё не прошла (REST биржи временно недоступен или прошло меньше минуты)'}.` : '—'}</p>
        <h3>REST биржи</h3>
        ...
        ${st.lastArchiveCheck ? `Сверка архива: ${st.lastArchiveCheck.ok ? 'совпадает' : '<span class="warn">расхождение</span>'} (${esc(st.lastArchiveCheck.detail)}, ${fmtDateTime(st.lastArchiveCheck.at)})` : 'Сверка архива: первая проверка — примерно через 11 минут после старта сервера (нужны два соседних 5-минутных блока).'}` : 'Supabase не подключён: постоянной истории нет.'}</p>
