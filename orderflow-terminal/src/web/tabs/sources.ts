    ...
    try {
      ...
      diagBox.innerHTML = `
        <div class="cards">
          ...
          <div class="card"><div class="k">Запись истории (${esc(store.key)})</div><div class="v">${!ps ? '—' : !ps.enabled ? '<span class="warn">не записывается</span>' : ps.lastError ? '<span class="warn">ошибка</span>' : 'ок'}</div><div class="k">${ps ? `очередь ${ps.queued}, несохранённый хвост ≈ ${Math.round(ps.oldestUnsavedMs / 1000)} с, отброшено ${ps.dropped}; последняя запись ${age(ps.lastOkAt)}` : ''}</div></div>
